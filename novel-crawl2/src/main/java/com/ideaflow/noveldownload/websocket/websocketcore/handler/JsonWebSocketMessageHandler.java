package com.ideaflow.noveldownload.websocket.websocketcore.handler;

import cn.hutool.core.util.StrUtil;
import cn.hutool.core.util.TypeUtil;
import cn.hutool.json.JSONUtil;
import com.ideaflow.noveldownload.websocket.websocketcore.listener.WebSocketMessageListener;
import com.ideaflow.noveldownload.websocket.websocketcore.message.JsonWebSocketMessage;
...
public class JsonWebSocketMessageHandler extends TextWebSocketHandler {
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        // 2.1 解析消息
        try {
            JsonWebSocketMessage jsonMessage = JSONUtil.toBean(message.getPayload(), JsonWebSocketMessage.class);
//            JsonWebSocketMessage jsonMessage = JsonUtils.parseObject(message.getPayload(), JsonWebSocketMessage.class);
            ...
            Object messageObj = JSONUtil.toBean(jsonMessage.getContent(), type, true);

            messageListener.onMessage(session, messageObj);
        } catch (Throwable e) {
            log.error("[handleTextMessage][session({}) message({}) 处理异常]", session.getId(), message.getPayload(),e);
        }
    }
